...
resetKeyStore | Boolean flag determining whether the user's key store should be reset (i.e. recreated from scratch) so that it contains only the given keys. Only works in combination with `keys` | option (default = `false`)
