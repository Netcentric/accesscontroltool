...
public class AceBeanValidatorImpl implements AceBeanValidator{
	...
	public boolean validate(final AceBean aceBean) throws AcConfigBeanValidationException{
		if(!enabled){
			return true;
		}
	...
	private boolean validate() throws AcConfigBeanValidationException{
		...
		if(!isActionOrPrivilegeDefined){
			String errorMessage = getBeanDescription(this.currentBeanCounter, this.aceBean.getPrincipalName()) + ", no actions or privileges defined" + "! Installation aborted!";
	...
	public boolean validateGlobbing(final AceBean tmpAclBean) throws InvalidRepGlobException {
		...
		if(Validators.isValidRegex(currentEntryValue)){
			...
			valid = false;
			String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + ",  invalid globbing expression: " + currentEntryValue;
			LOG.error(errorMessage);
	...
	public boolean validatePermission(final AceBean tmpAclBean) throws InvalidPermissionException {
		...
		if(Validators.isValidPermission(currentEntryValue)){
			...
			valid = false;
			String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + ", invalid permission: " + currentEntryValue;
			LOG.error(errorMessage);
	...
	public boolean validateActions(final AceBean tmpAclBean) throws InvalidActionException, TooManyActionsException, DoubledDefinedActionException {
		String principal = tmpAclBean.getPrincipalName();
		...
		if(actions.length > CqActionsMapping.ACTIONS_MAP.size()){
			String errorMessage = getBeanDescription (this.currentBeanCounter, principal) + " too many actions defined!";
			LOG.error(errorMessage);
		...
		for(int i = 0; i < actions.length; i++){
			...
			if(!Validators.isValidAction(actions[i])){
				String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + ", invalid action: " + actions[i];
				LOG.error(errorMessage);
				throw new InvalidActionException(errorMessage);
			}
			if(!actionsSet.add(actions[i])){
				String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + ", doubled defined action: " + actions[i];
				LOG.error(errorMessage);
	...
	public boolean validatePrivileges(final AceBean tmpAclBean) throws InvalidJcrPrivilegeException, DoubledDefinedJcrPrivilegeException {
		...
		for(int i = 0; i < privileges.length; i++){
			...
			if(!Validators.isValidJcrPrivilege(privileges[i])){
				String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + ",  invalid jcr privilege: " + privileges[i];
				LOG.error(errorMessage);
				throw new InvalidJcrPrivilegeException(errorMessage);
			}
			if(!privilegesSet.add(privileges[i])){
				String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + ", doubled defined jcr privilege: " + privileges[i];
				LOG.error(errorMessage);
	...
	public boolean validateAcePath(final AceBean tmpAclBean) throws InvalidPathException {
		...
		if(Validators.isValidNodePath(currentEntryValue)){
			...
			String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + ", invalid path: " + currentEntryValue;
			LOG.error(errorMessage);
	...
	public boolean validateAuthorizableId(final Set<String> groupsFromCurrentConfig, final AceBean tmpAclBean) throws NoGroupDefinedException, InvalidGroupNameException {
		boolean valid = true;
		String principal = tmpAclBean.getPrincipalName();
		...
		if(Validators.isValidAuthorizableId(principal)){
			...
			if(!groupsFromCurrentConfig.contains(principal)){
				String message = getBeanDescription(this.currentBeanCounter, principal) + " is not defined in group configuration";
			...
			valid = false;
			String errorMessage = getBeanDescription(this.currentBeanCounter, principal) + principal +", invalid authorizable name: " + principal;
			LOG.error(errorMessage);
	...
	@Override
	public void setGroupsFromCurrentConfig(final Set<String> groupsFromCurrentConfig) {
		this.groupsFromCurrentConfig = groupsFromCurrentConfig;
	}
	...
	private String getBeanDescription(final long beanCounter, final String principalName){
		return "Validation error while reading ACE definition nr." + beanCounter + " of authorizable: " + principalName;
	}

	@Override
	public void setCurrentAuthorizableName(final String name) throws AlreadyDefinedGroupException {
		if(this.enabled){
			LOG.info("insert {} into set", name);
			if(!alreadyProcessedAuthorizables.add(name)){
				String errorMessage = getBeanDescription(1, name) + ", found more than one ACE definition block for this group!";
				LOG.error(errorMessage);
