...
@Component(
		metatype = true,
		label = "AuthorizableCreatorService Service",
		description = "Service that installs groups according to textual configuration files")
...
public class AuthorizableCreatorServiceImpl implements AuthorizableCreatorService{
	...
	public void createNewAuthorizables(Map<String, LinkedHashSet<AuthorizableConfigBean>> principalMapFromConfig, final Session session, AcInstallationHistoryPojo status, AuthorizableInstallationHistory authorizableInstallationHistory) throws AccessDeniedException, UnsupportedRepositoryOperationException, RepositoryException, AuthorizableCreatorException{
		...
		for(String principalId : groupsFromConfigurations){
			...
			installAuthorizableConfigurationBean(session, tmpPricipalConfigBean, status, authorizableInstallationHistory);
		}

	}
	
	private void installAuthorizableConfigurationBean(final Session session, AuthorizableConfigBean principalConfigBean, AcInstallationHistoryPojo history, AuthorizableInstallationHistory authorizableInstallationHistory) throws AccessDeniedException,
	...
	AuthorizableExistsException, AuthorizableCreatorException {
		...
		if(userManager.getAuthorizable(principalId) == null) {
			createNewAuthorizable(principalConfigBean, history, authorizableInstallationHistory, userManager, vf);
		}
		// if current authorizable from config already exists in repository
		else{
			
			handleIntermediatePath(session, principalConfigBean, history, authorizableInstallationHistory, userManager);
			mergeGroup(history, authorizableInstallationHistory, principalConfigBean, userManager);
		}
	}
	...
	private void handleIntermediatePath(final Session session, AuthorizableConfigBean principalConfigBean,AcInstallationHistoryPojo history,
			AuthorizableInstallationHistory authorizableInstallationHistory, UserManager userManager)
		...
		if(existingAuthorizable.isGroup()){
			...
			if(!StringUtils.equals(intermediatedPathOfExistingGroup, principalConfigBean.getPath())){
				...
				message.append("group from config: " + principalConfigBean.getPrincipalID() + " has intermediate path: " + principalConfigBean.getPath()).append("\n");
				...
				ValueFactory vf = session.getValueFactory();
				Group newGroup = (Group)createNewAuthorizable(principalConfigBean, history,authorizableInstallationHistory,  userManager, vf);
				...
				deleteOldIntermediatePath(session, session.getNode(intermediatedPathOfExistingGroup));
	 ...
	 */
	private void deleteOldIntermediatePath(final Session session, Node oldIntermediateNode)
			throws RepositoryException, PathNotFoundException,
			VersionException, LockException, ConstraintViolationException,
			AccessDeniedException {
		
		// if '/home/groups' or '/home/users' was intermediatedNode, these must not get deleted!
		// also node to be deleted has to be empty, so no other authorizables stored under this path get deleted
		while(!StringUtils.equals("/home/groups", oldIntermediateNode.getPath()) && !StringUtils.equals("/home/users", oldIntermediateNode.getPath()) && !oldIntermediateNode.hasNodes()){
	...
	private void mergeGroup(AcInstallationHistoryPojo status,
			AuthorizableInstallationHistory authorizableInstallationHistory,AuthorizableConfigBean principalConfigBean,
	...
	private Authorizable createNewAuthorizable(AuthorizableConfigBean principalConfigBean,
			AcInstallationHistoryPojo status,
			AuthorizableInstallationHistory authorizableInstallationHistory,
		...
		if(isGroup){
			newAuthorizable = createNewGroup(userManager, principalConfigBean, status, authorizableInstallationHistory, vf, principalMapFromConfig );
			authorizableInstallationHistory.addNewCreatedAuthorizabe(principalId);
			LOG.info("Successfully created new Group: {}", principalId);
		} else{
			newAuthorizable = createNewUser(userManager, principalId, memberOf, password, status, authorizableInstallationHistory, vf, principalMapFromConfig );
	...
	private void mergeMemberOfGroups(String principalId,
		...
		//  transfer members to group in repo
			...
			mergeMemberOfGroupsFromConfig(principalId, status, userManager,membershipGroupsFromConfig);
	...
	private void mergeMemberOfGroupsFromRepo(String principalId,UserManager userManager, Set<String> membershipGroupsFromRepository)
			throws RepositoryException {
	...
	private void mergeMemberOfGroupsFromConfig(String principalId,AcInstallationHistoryPojo status, UserManager userManager,Set<String> membershipGroupsFromConfig) throws RepositoryException,
	AuthorizableExistsException, AuthorizableCreatorException {
		...
		Set<Authorizable> validatedGroups = validateAssignedGroups(userManager, principalId, membershipGroupsFromConfig.toArray(new String[0]));

		for(Authorizable membershipGroup : validatedGroups){
			LOG.info("{}: add authorizable to members of group {} in repository", principalId, membershipGroup.getID());
	...
	private void mergeMultipleMembersOfBothGroups(String principalId, AcInstallationHistoryPojo status, UserManager userManager, Set<String> membershipGroupsFromConfig,Set<String> membershipGroupsFromRepository)
		...
		if(membershipGroupsFromRepository.equals(membershipGroupsFromConfig)){
			...
			for(String authorizable : membershipGroupsFromRepository){
				// is current a also contained in memberOf-groups property of existing group?
				if(membershipGroupsFromConfig.contains(authorizable)){
					...
					LOG.info("delete {} from members of group {} in repository", principalId, authorizable);
			...
			for(Authorizable authorizable : validatedGroups){
				...
				if(membershipGroupsFromRepository.contains(authorizable.getID())){
					continue;
				}else{

					// if not add that group to membersOf-property of existing group

					LOG.info("add {} to members of group {} in repository", principalId, authorizable);
	...
	private Authorizable createNewGroup(final UserManager userManager, AuthorizableConfigBean principalConfigBean, AcInstallationHistoryPojo status, AuthorizableInstallationHistory authorizableInstallationHistory, ValueFactory vf, Map<String, LinkedHashSet<AuthorizableConfigBean>> principalMapFromConfig )
		...
		Group newGroup = null;
		try{
			newGroup = userManager.createGroup(new PrincipalImpl(groupID), intermediatePath);
		...
		if(memberOf != null && memberOf.length > 0){
			Set<Authorizable> assignedGroups = validateAssignedGroups(userManager, groupID, memberOf);
	...
	private Authorizable createNewUser(final UserManager userManager, final String userID, final String[] memberOf,
			final String password,  AcInstallationHistoryPojo status, AuthorizableInstallationHistory authorizableInstallationHistory, ValueFactory vf, Map<String, LinkedHashSet<AuthorizableConfigBean>> principalMapFromConfig ) throws AuthorizableExistsException,
			RepositoryException, AuthorizableCreatorException {
		User newUser = null;
		if(memberOf != null && memberOf.length > 0){

			// add group to groups according to configuration
			Set<Authorizable> authorizables = validateAssignedGroups(userManager, userID, memberOf);
	 ...
	 */
	private Set<Authorizable>  validateAssignedGroups(final UserManager userManager, final String authorizablelID,
			final String[] memberOf) throws RepositoryException, AuthorizableCreatorException {
		...
		for(String principal : memberOf){
			...
			if(authorizable != null){
				...
				// check if authorizable is contained in any of the configurations
				if(principalMapFromConfig.keySet().contains(principal)){

					// get authorizable intermediatePath
					LinkedHashSet<AuthorizableConfigBean> authorizableConfigSet = principalMapFromConfig.get(principal);
					...
					//if authorizableConfigBean.getPath() returns an empty string (no path defined in configuration) the standard path gets used
					Authorizable newGroup = userManager.createGroup(new PrincipalImpl(principal), authorizableConfigBean.getPath());
