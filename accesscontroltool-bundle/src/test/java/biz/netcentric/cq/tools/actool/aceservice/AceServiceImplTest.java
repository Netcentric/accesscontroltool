...
public class AceServiceImplTest extends CqTest{
	...
	@Before
	public void setup() throws AccessDeniedException, ItemExistsException, ReferentialIntegrityException, ConstraintViolationException, InvalidItemStateException, VersionException, LockException, NoSuchNodeTypeException, RepositoryException {
	...
	private Node setUpProjectNode(Node acToolNode, String projectNodeName) throws ItemExistsException, PathNotFoundException, NoSuchNodeTypeException, LockException, VersionException, ConstraintViolationException, RepositoryException{
		Node node = acToolNode.addNode(projectNodeName,"nt:folder");
		node.addNode(projectNodeName + "_0.txt", "nt:file").addNode("jcr:content", "nt:resource").setProperty("jcr:data", ".");;
		this.session.save();
		node.addNode(projectNodeName + "_1.txt", "nt:file").addNode("jcr:content", "nt:resource").setProperty("jcr:data", ".");;
		this.session.save();
		node.addNode(projectNodeName + "_2.txt", "nt:file").addNode("jcr:content", "nt:resource").setProperty("jcr:data", ".");
		this.session.save();
		return node;
	}
	...
	public void getNewestConfiguationNodesTest() throws Exception{
		...
		Map<String, String> configNodes = aceService.getNewestConfigurationNodes(configurationRootPath, session, new AcInstallationHistoryPojo());
